...
('admin@leetcode.com', '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J8K8K8K8K', 'Admin User', 'admin');

-- Insert sample problems
INSERT INTO problems (title, description, difficulty, company, categories, tags, status, created_by) VALUES 
('Design Database', 'You need to design a simple database system that supports the following functionalities: Insert: Add a key-value pair to the database. Remove: Delete a key from the database. Retrieve: Fetch the value associated with a given key. Optional: Support additional functionalities like listing all keys or updating a value.', 'medium', 'Google', ARRAY['coding', 'phone', 'onsite'], ARRAY['database', 'design', 'system'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('Array Compression', 'Implement an array compression algorithm that reduces the size of arrays by removing consecutive duplicate elements and replacing them with a count.', 'medium', 'Microsoft', ARRAY['coding', 'phone'], ARRAY['array', 'compression', 'algorithm'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('Backend Assignment Interview', 'Design and implement a backend system for a social media platform with user authentication, post creation, and feed generation.', 'hard', 'Facebook', ARRAY['coding', 'onsite'], ARRAY['backend', 'system', 'design'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('Basic Calculator', 'Implement a basic calculator that can evaluate mathematical expressions containing +, -, *, /, and parentheses.', 'easy', 'Amazon', ARRAY['coding', 'phone'], ARRAY['calculator', 'math', 'expression'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('LLM Token Counter', 'Create a token counting system for Large Language Models that accurately counts tokens in text input.', 'medium', 'OpenAI', ARRAY['coding', 'phone'], ARRAY['llm', 'token', 'counting'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('Neural Network Optimizer', 'Implement an optimizer for neural networks that can handle gradient descent with momentum and adaptive learning rates.', 'hard', 'DeepMind', ARRAY['coding', 'onsite'], ARRAY['neural', 'network', 'optimizer'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('Prompt Engineering Framework', 'Design a framework for managing and optimizing prompts for AI models with version control and A/B testing capabilities.', 'medium', 'Anthropic', ARRAY['coding', 'phone'], ARRAY['prompt', 'engineering', 'framework'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com')),
('Vector Database Query', 'Implement a vector similarity search system for high-dimensional data with efficient indexing and querying.', 'hard', 'Pinecone', ARRAY['coding', 'onsite'], ARRAY['vector', 'database', 'similarity'], 'published', (SELECT id FROM users WHERE email = 'admin@leetcode.com'));
